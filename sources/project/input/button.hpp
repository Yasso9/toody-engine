...
class Button final : public sf::Drawable
{
  public:
    ...
    /* Regarde si un bouton est pressé et si oui, retourne le numero du nouveau
    bouton pressé, sinon cela retourne le même bouton, à utiliser dans les
    fonctions update de state lorsqu'on veut actualiser les boutons */
    unsigned int update_press( sf::Vector2f const & position,
                               bool const & inputIsPress );
    /* Met a jour le rendu de selection si la souris passe par dessus le
    bouton, de même que update_press, à utiliser dans les fonctions updates*/
