...
namespace OpenGL
{
    void graphics()
    {
        ...
        while ( ! exit )
        {
            ...
            if ( ! backgroundTexture.loadFromFile( too::get_resources_path()
                                                   + "background.jpg"s ) )
                return;
            ...
            sf::Font font;
            if ( ! font.loadFromFile( too::get_resources_path() + "tuffy.ttf"s ) )
                return;

            sf::Text text( "SFML / OpenGL demo"s, font );
            sf::Text sRgbInstructions( "Press space to toggle sRGB conversion"s,
                                       font );
            sf::Text mipmapInstructions( "Press return to toggle mipmapping"s,
                                         font );
            ...
            sf::Texture texture;
            if ( ! texture.loadFromFile( too::get_resources_path()
                                         + "logo.png"s ) )
            {
                std::cerr << " Error : Texture not loaded"s << std::endl;
                return;
            }
            ...
            {
                std::cerr << "Error : MipMap not generated"s << std::endl;
            }

            // Make the window the active window for OpenGL calls
            if ( ! window.setActive( true ) )
            {
                std::cerr << " Error : Failed to set window to active"s
                          << std::endl;
                return;
            }
            ...
            {
                std::cerr << "Failed to set window to inactive"s << std::endl;
                return;
            }
            ...
            while ( window.isOpen() )
            {
                ...
                while ( window.pollEvent( event ) )
                {
                    ...
                    if ( event.type == sf::Event::Resized )
                    {
                        ...
                        if ( ! window.setActive( true ) )
                        {
                            std::cerr << "Failed to set window to active"s
                                      << std::endl;
                            return;
                        }
                        ...
                        if ( ! window.setActive( false ) )
                        {
                            std::cerr << "Failed to set window to inactive"s
                                      << std::endl;
                            return;
                        }
                ...
                glRotatef( clock.getElapsedTime().asSeconds() * 50.f,
                           1.f,
                           0.f,
                           0.f );
                glRotatef( clock.getElapsedTime().asSeconds() * 30.f,
                           0.f,
                           1.f,
                           0.f );
                glRotatef( clock.getElapsedTime().asSeconds() * 90.f,
                           0.f,
                           0.f,
                           1.f );
